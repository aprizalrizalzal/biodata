document
  ...
  .addEventListener("submit", function (e) {
    ...
    if (!phone.match(/^[0-9]{10,15}$/)) {
      document.getElementById("error-phone").innerText =
        "No. telepon harus 10-15 digit angka.";
      valid = false;
    }
    if (!messages) {
      document.getElementById("error-messages").innerText =
    ...
    if (valid) {
      document.getElementById(
        "form-success"
      ).innerText = `Halo, ${name}! Terima kasih sudah menghubungi.`;
      document.getElementById("form-success").className = "success";
      // this.reset(); // Uncomment jika ingin reset form setelah submit
    }
  });

// Responsive textarea cols for mobile
...
function renderTugas() {
  ...
  for (let i = 0; i < listTugas.length; i++) {
    ...
    output += `<li>
                    ...
                    <button onclick="tandaiSelesai(${i})" ${
      item.selesai ? "disabled" : ""
    }>Selesai</button>
                </li>`;
  }
